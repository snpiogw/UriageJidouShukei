...
class AggregationProfileValidationServiceTest {
    ...
    void allowsDifferentInputAndOutputSheetsInTheSameSpreadsheet() {
        ...
        when(repository.findConflicts("same-sheet", -1L)).thenReturn(List.of(other));

        validation.validate(command("店舗B", "same-sheet", "売上B", "結果B", "エラーB"), -1L);
    }
    ...
    void rejectsDuplicateColumnNamesAndInvalidTimeZone() {
        ...
        ProfileCommand invalidZone = new ProfileCommand("店舗", "sheet", "入力", "結果", "エラー",
